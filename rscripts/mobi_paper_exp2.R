...
# Plot data
  ...
  p <- p + geom_line(aes(x = TIME, y = DV),
    data = plotdf, colour = "black", size = 0.8, linetype = "dashed")
  
  p <- p + geom_line(aes(x = TIME, y = PREDnd), 
    data = plotdf, colour = "#D55E00", size = 0.8)  #, linetype = "dashed")
  p <- p + geom_line(aes(x = TIME, y = PREDhi), 
    data = plotdf, colour = "#0072B2", size = 0.8)  #, linetype = "dashed")
  p <- p + geom_line(aes(x = TIME, y = PREDav),
    data = plotdf, colour = "#009E73", size = 0.8)  #, linetype = "dashed")
...
# Produce Figure4 for the MoBi methods paper
  ggsave("produced_data/mobi_paper_exp2.png", width = 17.4, height = 10.4, 
    units = c("cm"))
  # ggsave("produced_data/Figure4.eps", width = 17.4, height = 23.4, 
