...
# Source functions, data and models
  ...
  source(paste(git.dir, reponame, "models", "single_tissues",
    "flow_limited_cubic.R", sep = "/"))
  source(paste(git.dir, reponame, "models", "single_tissues",
    "memb_limited_cubic.R", sep = "/"))
...
# Ensure that doses 0.5 and 1.5 don't go over 300 mins
  ...
  p <- p + geom_line(aes(x = time, y = Cart), colour = "red")
  # p <- p + geom_line(aes(x = time, y = Cven), colour = "blue")
  # p <- p + geom_line(aes(x = time, y = Ctis), colour = "green4")
