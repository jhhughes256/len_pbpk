...
# Check subject numbers
  with(dataip, table(ID))
  with(dataip, table(UID))

# Check PK dose data
  with(dataip, table(DOSEMGKG, TADNOM))

# Check distribution of DV
  po.distplot(dataip, "alldata", plot.out)
  po.distplot(dataip.av, "meandata", plot.out)

# Calculate dose normalized concentrations and mark missing DV
# Units are ng/ml per mg
  dataip$DVNORM <- dataip$DV/dataip$DOSEMG
  dataip$MDV <- ifelse(is.na(dataip$DV), 1, 0)
  dataip.av$DVNORM <- dataip.av$DV/dataip.av$DOSEMG
  dataip.av$MDV <- ifelse(is.na(dataip.av$DV), 1, 0)

  po.CvTplot(dataip, "alldata", plot.out)
  po.CvTplot(dataip.av, "meandata", plot.out)
  po.CvTplot(dataip, "alldata", plot.out, dosenorm = T)
