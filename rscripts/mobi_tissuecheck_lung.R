...
# Rename columns
  ...
  names(bralo) <- c(
    ...
    paste0(
      ...
      c(rep(c("Venous", "Arterial"), each = 2), rep("Lung", times = 5))
    )
  )
  
# Convert time to minutes
  bralo$TIME <- bralo$TIME*60
  bralo$RES <- bralo$tisLung - pksim$tisLung
  bralo$PROPRES <- with(bralo, RES/tisLung)
  
# Read in MoBi data for fixed Lung simulation
  model.name <- "PKSimLungFix.xlsx"
...
# Convert time to minutes
  ...
  brahi$RES <- brahi$tisLung - pksim$tisLung
...
# Convert time to minutes
  ...
  brami$RES <- brami$tisLung - pksim$tisLung
  brami$PROPRES <- with(brami, RES/tisLung)
  
# Plot Lung concentrations against each other
  # p <- NULL
  # p <- ggplot()
  # p <- p + geom_line(aes(x = TIME, y = tisLung), data = pksim, colour = "red")
  # p <- p + geom_line(aes(x = TIME, y = tisLung), data = bralo, linetype = "dashed")
  # p <- p + geom_line(aes(x = TIME, y = tisLung), data = brahi, linetype = "dashed")
  # p <- p + geom_line(aes(x = TIME, y = tisLung), data = brami, linetype = "dotted", colour = "blue")
  # p <- p + scale_y_log10()
  # p
