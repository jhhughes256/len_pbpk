...
# Set ggplot2 theme
  theme_bw2 <- theme_set(theme_bw(base_size = 18))
...
# Plot data for third plot
  ...
  p <- p + geom_line(aes(x = TIME, y = PREDnd), 
    data = plotdf2[plotdf2$TISSUE == "Lung",], colour = "#D55E00", size = 0.8)  #, linetype = "dashed")
  p <- p + geom_line(aes(x = TIME, y = PREDhi), 
    data = plotdf2[plotdf2$TISSUE == "Lung",], colour = "#0072B2", size = 0.8)  #, linetype = "dashed")
  # p <- p + geom_line(aes(x = TIME, y = PREDav),
  #   data = plotdf2[plotdf2$TISSUE == "Lung",], colour = "#009E73", size = 0.8)  #, linetype = "dashed")
  
  p <- p + geom_point(aes(x = TIME, y = FFlo), 
    data = plotdf[plotdf$TISSUE == "Lung",], colour = "#D55E00", shape = 4, stroke = 1.5, size = 2)
  p <- p + geom_point(aes(x = TIME, y = FFhi), 
    data = plotdf[plotdf$TISSUE == "Lung",], colour = "#0072B2", shape = 4, stroke = 1.5, size = 2)
  # p <- p + geom_point(aes(x = TIME, y = IF), 
  #   data = plotdf[plotdf$TISSUE == "Lung",], colour = "#009E73", shape = 4, stroke = 0.8, size = 1)

  p <- p + geom_line(aes(x = TIME, y = FF),
    data = plotdf[plotdf$TISSUE == "Lung",], colour = "black", size = 0.8, linetype = "dashed")
  p <- p + xlab("\nTime (min)")
  if ("Lung" == "Lung") {
    p <- p + ylab(NULL)
  } else {
  ...
  p <- p + coord_cartesian(xlim = c(0, 0.6), ylim = NULL)
  p <- p + facet_wrap(~TISSUE, ncol = 2, scale = "free_y")
  p
  
  ggsave("produced_data/paganz_fig3.png", width = 2.6, height = 2.6)
  
  p <- p + geom_line(aes(x = TIME, y = PREDav),
    data = plotdf2[plotdf2$TISSUE == "Lung",], colour = "#009E73", size = 0.8)  #, linetype = "dashed")
  p <- p + geom_point(aes(x = TIME, y = IF),
    data = plotdf[plotdf$TISSUE == "Lung",], colour = "#009E73", shape = 4, stroke = 1.5, size = 2)
  p

  ggsave("produced_data/paganz_fig4.png", width = 2.6, height = 2.6)