...
# Rename columns
  names(braindata) <- c(
    ...
    "Tissue Blood Cells", "Tissue Interstitial", "Tissue Intracellular", 
    "Whole Tissue"
  )
  
# Read in simulation data
  file.dir <- "raw_data/MoBi_paper/"
  model.name <- "HeartSumexp13.xlsx"
...
# Read in simulation data
  file.dir <- "raw_data/MoBi_paper/"
  model.name <- "KidneySumexp13.xlsx"
...
# Blood flows of each tissue are
  ...
  simdata$Cp2t <- with(simdata, PREDp*K/tickRate)
  simdata$Cb2t <- with(simdata, PREDb*K/tickRate)
  ...
  p1 <- NULL
  p1 <- ggplot()
  p1 <- p1 + geom_line(aes(x = TIME, y = RESp, colour = TISSUE), data = simdata[-1,])
