...
# Rename columns
  ...
  names(bralo) <- c(
    ...
    paste0(
      ...
      c(rep(c("Venous", "Arterial"), each = 2), rep("Brain", times = 5))
    )
  )
  
# Convert time to minutes
  bralo$TIME <- bralo$TIME*60
  bralo$RES <- bralo$tisBrain - pksim$tisBrain
  bralo$PROPRES <- with(bralo, RES/tisBrain)
  
# Read in MoBi data for fixed Brain simulation
  model.name <- "PKSimBrainFix.xlsx"
...
# Convert time to minutes
  ...
  brahi$RES <- brahi$tisBrain - pksim$tisBrain
  brahi$PROPRES <- with(brahi, RES/tisBrain)
  
# Read in MoBi data when proportional gain uses half the elim. rate const.
  model.name <- "PKSimBrainHalf.xlsx" #"PKSimBrainEst.xlsx"
...
# Convert time to minutes
  ...
  brami$RES <- brami$tisBrain - pksim$tisBrain
  brami$PROPRES <- with(brami, RES/tisBrain)
  
# Plot Brain concentrations against each other
  # p <- NULL
  # p <- ggplot()
  # p <- p + geom_line(aes(x = TIME, y = tisBrain), data = pksim, colour = "red")
  # p <- p + geom_line(aes(x = TIME, y = tisBrain), data = bralo, linetype = "dashed")
  # p <- p + geom_line(aes(x = TIME, y = tisBrain), data = brahi, linetype = "dashed")
  # p <- p + geom_line(aes(x = TIME, y = tisBrain), data = brami, linetype = "dotted", colour = "blue")
  # p <- p + scale_y_log10()
  # p
