...
# Define forcing functions
  ...
  tissue1est2 <- function(t) {
    exp(-3.95E-3*t - 5.60) + exp(-2.09E-2*t - 0.286) +
    exp(-0.050*t + 1.85) - exp(-0.12*t)*sum(exp(c(-5.6, -0.286, 1.85)))
  }
...
# Create plot
  fig1plot <- function(obs, est1, est2) {
    p <- NULL
    p <- ggplot()
    p <- p + geom_line(aes(x = TIME, y = get(est1)), data = estdata,
      size = 0.8, colour = "#D55E00")
    p <- p + geom_line(aes(x = TIME, y = get(est2)), data = estdata,
      size = 0.8, colour = "#0072B2")
  ...
  p1 <- fig1plot("OBS1", "EST1P1", "EST1P2")
  p2 <- fig1plot("OBS2", "EST2P1", "EST2P2")
