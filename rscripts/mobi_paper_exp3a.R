...
# Plot data for first plot row
  plotr1fn <- function(cond) {
    ...
    p <- p + geom_line(aes(x = TIME, y = FFlo), 
      data = plotdf[cond,], colour = "#D55E00", size = 0.8)  #, linetype = "dashed")
  
    p <- p + geom_line(aes(x = TIME, y = FF),
      data = plotdf[cond,], colour = "black", size = 0.8, linetype = "dashed")
    p <- p + xlab("\nTime (min)")
  ...
  p1 <- plotr1fn(plotdf$TISSUE == "Brain")
  p2 <- plotr1fn(plotdf$TISSUE == "Lung") 
...
# Plot data for second plot
  plotr2fn <- function(cond, ylim) {
    ...
    p <- p + geom_point(aes(x = TIME, y = FFlo), 
      data = plotdf[cond,], colour = "#D55E00", shape = 4, stroke = 1, size = 1)
    p <- p + geom_point(aes(x = TIME, y = FFhi), 
      data = plotdf[cond,], colour = "#0072B2", shape = 4, stroke = 1, size = 1)
    p <- p + geom_point(aes(x = TIME, y = IF), 
      data = plotdf[cond,], colour = "#009E73", shape = 4, stroke = 1, size = 1)
  ...
  p3 <- plotr2fn(plotdf$TISSUE == "Brain", c(3, 6.5))
  # p3 <- plotr2fn(plotdf$TISSUE == "Brain", NULL)
  p4 <- plotr2fn(plotdf$TISSUE == "Lung", NULL) 
...
# Plot data for third plot
  ...
  plotr3fn <- function(cond1, cond2, ylim) {
    ...
    p <- p + geom_point(aes(x = TIME, y = FFlo), 
      data = plotdf[cond1,], colour = "#D55E00", shape = 4, stroke = 0.8, size = 1)
    p <- p + geom_point(aes(x = TIME, y = FFhi), 
      data = plotdf[cond1,], colour = "#0072B2", shape = 4, stroke = 0.8, size = 1)
    p <- p + geom_point(aes(x = TIME, y = IF), 
      data = plotdf[cond1,], colour = "#009E73", shape = 4, stroke = 0.8, size = 1)
  ...
  p5 <- plotr3fn(plotdf$TISSUE == "Brain", plotdf2$TISSUE == "Brain", c(4.9, 7.4))
  p6 <- plotr3fn(plotdf$TISSUE == "Lung", plotdf2$TISSUE == "Lung", NULL) 
  
# Plot together using cowplot
  plot_grid(p1, p2, p3, p4, p5, p6, align = "hv", ncol = 2, 
    labels = letters[1:6], hjust = c(-6, -5.5, -6, -5.3, -6, -11))
  
# Produce Figure4 for the MoBi methods paper
  ggsave("produced_data/mobi_paper_exp3.png", width = 17.4, height = 19.4,
    units = c("cm"))
