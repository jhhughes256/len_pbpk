...
# Read in simulation data and rename columns
  ...
  model.name <- "PKSimKidneyAve.xlsx"
  kidney_raw <- read_excel(paste0(file.dir, model.name))
  kidney <- as.data.frame(kidney_raw)
  names(kidney) <- c(
    "TIME", 
    paste0(
      c(rep(c("pl", "bc"), times = 3), "is", "ic", "uri", "ti"),
      c(rep(c("Efferent", "Afferent"), each = 2), rep("Tissue", times = 6))
    )
  )
  
  model.name <- "PKSimLungAve.xlsx"
